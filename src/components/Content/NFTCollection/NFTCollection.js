...
const NFTCollection = () => {
  ...
  const makeOfferHandler = (event, id, key) => {
    ...
    .on('receipt', (receipt) => {      
      ...
      .on('error', (error) => {
        window.alert('Blok zincirine eklerken bir şeyler ters gitti');
  ...
  return(
    <div className="row text-center">
      {collectionCtx.collection.map((NFT, key) => {
        ...
        return(
          <div key={key} className="col-md-2 m-3 pb-3 card shadow-lg border-0">
              ...
              owner !== web3Ctx.account ?
                <div className="row">
                  <div className="d-grid gap-2 col-5 mx-auto">
                    <button onClick={buyHandler} value={index} className="btn btn-success">SATIN AL</button>
                  </div>
                ...
                <div className="row">
                  <div className="d-grid gap-2 col-5 mx-auto">
                    <button onClick={cancelHandler} value={index} className="btn btn-danger">İPTAL</button>
                  </div>
                  <div className="col-7 d-flex justify-content-end">
                    <img src={eth} width="25" height="25" className="align-center float-start" alt="price icon"></img>                
                    <p className="text-start"><b>{`${price}`}</b></p>
              ...
              owner === web3Ctx.account ?              
                <form className="row g-2" onSubmit={(e) => makeOfferHandler(e, NFT.id, key)}>                
                  <div className="col-5 d-grid gap-2">
                    <button type="submit" className="btn btn-secondary">TEKLİF</button>
                  </div>
                  <div className="col-7">
                    <input
                      type="number"
